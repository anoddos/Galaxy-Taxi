...
public class AccountService : IAccountService
{
	...
	private async Task ValidateNewPassword(string OldPassword, string NewPassword, CallContext context = default)
	{
		...
		if (!BCrypt.Net.BCrypt.Verify(OldPassword, account.PasswordHash)) throw new RpcException(new Status(StatusCode.InvalidArgument, "Old Password Is Incorrect"));
		if (BCrypt.Net.BCrypt.Verify(NewPassword, account.PasswordHash)) throw new RpcException(new Status(StatusCode.AlreadyExists, "New Password Can't be the same as the old password"));
	}
	...
	public async Task UpdateAccountSettings(UpdateAccountSettingsRequest request, CallContext context = default)
	{
		...
		if (!string.IsNullOrWhiteSpace(request.AccountInformation?.CompanyName) && request.AccountInformation?.CompanyName != account?.CompanyName)
		{
		...
		if (!string.IsNullOrWhiteSpace(request.AccountInformation?.Email) && request.AccountInformation?.Email != account?.Email)
		{
	...
	public async Task<AccountSettings?> GetAccountSettings(CallContext context = default)
	{
		...
		if (loggedInAs == AccountType.CustomerCompany)
		{
			...
			if (company == null) throw new RpcException(new Status(StatusCode.NotFound, "Customer company does not exists"));
			maxAmountPerEmployee = company.MaxAmountPerEmployee;
		}
		var account = await _db.Accounts.SingleOrDefaultAsync(a => a.Id == accountId);

		return account != null ? new AccountSettings
									{ 
										CompanyName = account.CompanyName, 
										Email = account.Email, 
										MaxAmountPerEmployee = maxAmountPerEmployee,
										IsCustomerCompany = loggedInAs == AccountType.CustomerCompany} : null;
	}
}